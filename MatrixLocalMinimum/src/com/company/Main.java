...
public class Main {
    ...
    static class MyMatrix {
        ...
        public void findMaxLocalMaximum() {
            HashSet<Integer> minimums = new HashSet<>();

            for (int row = 0; row < matrix.length; row++) {
                for (int col = 0; col < matrix.length; col++) {
                    if (isLocalMinumum(row, col)) {
                        minimums.add(matrix[row][col]);
                    }
                }
            }
            ...
            if (minimums.size() != 0) {
                noAnswer = false;
                maxLocalMaximum = minimums.size();
            } else {
                noAnswer = true;
            }
        }

        private boolean isLocalMinumum(int elementRow, int elementCol) throws NoSuchElementException {
